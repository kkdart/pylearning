...
engine = create_engine('postgresql://postgres:pwtesting@10.1.1.40:5432/books')
...
class BookAuthor(Base):
	...
	def __repr__(self):
		return '''<BookAuthor(bookauthor_id='{0}', 
			...
			book_title='{3}'>'''.format(self.bookauthor_id, self.author.first_name, 
...
def add_book(title, number_of_pages, first_name, last_name):
	...
	try:
		existing_author = session.query(Author).filter(Author.first_name == first_name, Author.last_name == last_name)

		session.add(book)
		if existing_author is not None:
			session.flush()
			pairing = BookAuthor(author_id=existing_author.first().author_id, book_id=book.book_id)
		else:
			session.add(author)
			session.flush()
